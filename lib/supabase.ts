...
// Clients are lazily created so missing env vars don't crash the build.
...
export const supabase: SupabaseClient = new Proxy({} as SupabaseClient, {
  get: (_t, prop) => {
    if (!_supabase) _supabase = getClient('anon')
...
export const supabaseAdmin: SupabaseClient = new Proxy({} as SupabaseClient, {
  get: (_t, prop) => {
    if (!_supabaseAdmin) _supabaseAdmin = getClient('service')
